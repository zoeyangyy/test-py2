...
r=requests.get('http://opaclibrary.sufe.edu.cn/opac/search_adv_result.php?sType0=any&q0=java&with_ebook=&page=1')
...
for row in table:
	...
	print(j)
'''
link = soup.find_all("a",class_="blue")

for item in link:
	p=requests.get('http://opaclibrary.sufe.edu.cn/opac/'+item['href'])
	print(p.url)
	soup1=BeautifulSoup(p.text)
	print(len(soup1.find_all(attrs={"color":"green"})))
'''